...
class Installer implements JsonSerializable {
	...
	public function __construct($installData = array()) {
		...
		if ($step === false || $step === null) {
			log_install_medium('step', "Resetting to STEP_WELCOME as not found");
	...
	private function setEula($param_eula = '') {
		if ($param_eula == 'Accepted') {
			$param_eula = '1';
		}

		if (!is_numeric($param_eula)) {
			$this->addError(Installer::STEP_WELCOME, 'Eula', 'Failed to apply specified Eula: ' . $param_eula);
		} else {
	...
	private function setPaths($param_paths = array()) {
		...
		if (is_array($param_paths)) {
			log_install_debug('paths', "setPaths($this->stepCurrent)");
			...
			foreach ($param_paths as $name => $path) {
				if (array_key_exists($name, $this->paths)) {
					...
					log_install_high('paths', "setPaths(): $name => $path, $should_set");
					log_install_debug('paths', clean_up_lines(var_export($this->paths[$name], true)));
				}
			}
		}
	}
...
//						}
	...
	public static function processInstall($backgroundArg) {
		...
		if ("$backgroundTime" != "$backgroundArg" && "-b" != "$backgroundArg") {
			...
			log_install_always('', __('Background was already started at %s, this attempt at %s was skipped',
