...
function draw_navigation_text($type = "url") {
	...
	if (isset($_REQUEST['tree_id']) || isset($_REQUEST['leaf_id'])) {
		...
		if (isset($_REQUEST['leaf_id']) && $_REQUEST['leaf_id'] != '') {
			$leaf = db_fetch_row("SELECT * FROM graph_tree_items WHERE id=" . $_REQUEST['leaf_id']);

			if (sizeof($leaf)) {
				if ($leaf['host_id'] > 0) {
					$leaf_name = db_fetch_cell('SELECT description FROM host WHERE id=' . $leaf['host_id']);
				}else{
					$leaf_name = $leaf['title'];
				}

				$tree_name = db_fetch_cell('SELECT name FROM graph_tree WHERE id=' . $leaf['graph_tree_id']);
			}else{
				$leaf_name = 'Leaf';
			}
			...
			if (isset($_REQUEST['host_group_data']) && $_REQUEST['host_group_data'] != '') {
				...
				if ($parts[0] == 'graph_template') {
					$leaf_sub = db_fetch_cell('SELECT name FROM graph_templates WHERE id=' . $parts[1]);
				}else{
					if ($parts[1] > 0) {
						$leaf_sub = db_fetch_cell('SELECT name FROM snmp_query WHERE id=' . $parts[1]);
					}else{
			...
			if (isset($_REQUEST['tree_id'])) {
				$tree_name = db_fetch_cell('SELECT name FROM graph_tree WHERE id=' . $_REQUEST['tree_id']);
			}else{
				$tree_name = '';
			}
		}
