...
function xml_to_graph_template($hash, &$xml_array, &$hash_cache, $hash_version) {
	...
	$_graph_template_id = db_fetch_cell_prepared('SELECT id FROM graph_templates WHERE hash = ?', array($hash));
	...
	if (is_array($xml_array['items'])) {
		while (list($item_hash, $item_array) = each($xml_array['items'])) {
			...
			$_graph_template_item_id = db_fetch_cell_prepared('SELECT id 
				...
				AND local_graph_id=0', array($parsed_hash['hash'], $graph_template_id));
			...
			while (list($field_name, $field_array) = each($struct_graph_item)) {
				...
				if (isset($item_array[$field_name])) {
					...
					if (preg_match('/hash_([a-f0-9]{2})([a-f0-9]{4})([a-f0-9]{32})/', $item_array[$field_name])) {
						...
						if (empty($color_id)) {
							db_execute_prepared('INSERT INTO colors (hex) VALUES (?)', array($item_array[$field_name]));
	...
	if (is_array($xml_array['inputs'])) {
		while (list($item_hash, $item_array) = each($xml_array['inputs'])) {
			...
			$_graph_template_input_id = db_fetch_cell_prepared('SELECT id 
				...
				WHERE hash = ?
				AND graph_template_id = ?', array($parsed_hash['hash'], $graph_template_id));
				...
				for ($i=0; $i<count($hash_items); $i++) {
					...
					if (isset($hash_cache['graph_template_item']{$parsed_hash['hash']})) {
						db_execute_prepared('REPLACE INTO graph_template_input_defs 
							...
							VALUES (?, ?)', array($graph_template_input_id, $hash_cache['graph_template_item']{$parsed_hash['hash']}));
					}
				}
			}
		}
	...
	if (is_array($xml_array['items'])) {
		while (list($item_hash, $item_array) = each($xml_array['items'])) {
			...
			$_data_template_rrd_id = db_fetch_cell_prepared('SELECT id 
				...
				AND local_data_id=0', array($parsed_hash['hash'], $data_template_id));
...
function xml_to_data_query($hash, &$xml_array, &$hash_cache) {
	...
	$_data_query_id = db_fetch_cell_prepared('SELECT id FROM snmp_query WHERE hash = ?', array($hash));
	...
	if (is_array($xml_array['graphs'])) {
		while (list($item_hash, $item_array) = each($xml_array['graphs'])) {
			...
			$_data_query_graph_id = db_fetch_cell_prepared('SELECT id 
				...
				WHERE hash = ?
				AND snmp_query_id = ?', array($parsed_hash['hash'], $data_query_id));
			...
			if (is_array($item_array['sv_graph'])) {
				while (list($sub_item_hash, $sub_item_array) = each($item_array['sv_graph'])) {
					...
					$_data_query_graph_sv_id = db_fetch_cell_prepared('SELECT id 
						...
						WHERE hash = ?
						AND snmp_query_graph_id = ?', array($parsed_hash['hash'], $data_query_graph_id));
...
function xml_to_gprint_preset($hash, &$xml_array, &$hash_cache) {
	...
	$_gprint_preset_id = db_fetch_cell_prepared('SELECT id FROM graph_templates_gprint WHERE hash = ?', array($hash));
...
function xml_to_host_template($hash, &$xml_array, &$hash_cache) {
	...
	/* import into: graph_templates_gprint */
	$_host_template_id = db_fetch_cell_prepared('SELECT id FROM host_template WHERE hash = ?', array($hash));
	...
	if (!empty($hash_items[0])) {
		for ($i=0; $i<count($hash_items); $i++) {
			...
			if (isset($hash_cache['graph_template']{$parsed_hash['hash']})) {
				db_execute_prepared('REPLACE INTO host_template_graph (host_template_id,graph_template_id) values (?, ?)', array($host_template_id, $hash_cache['graph_template']{$parsed_hash['hash']}));
			}
		}
	}
		...
		for ($i=0; $i<count($hash_items); $i++) {
			...
			if (isset($hash_cache['data_query']{$parsed_hash['hash']})) {
				db_execute_prepared('REPLACE INTO host_template_snmp_query 
					...
					VALUES (?, ?)', array($host_template_id, $hash_cache['data_query']{$parsed_hash['hash']}));
			}
		}
	}
...
function xml_to_cdef($hash, &$xml_array, &$hash_cache) {
	...
	/* import into: cdef */
	$_cdef_id = db_fetch_cell_prepared('SELECT id FROM cdef WHERE hash = ?', array($hash));
	...
	if (is_array($xml_array['items'])) {
		while (list($item_hash, $item_array) = each($xml_array['items'])) {
			...
			unset($save);
			$_cdef_item_id = db_fetch_cell_prepared('SELECT id FROM cdef_items WHERE hash= ? AND cdef_id = ?', array($parsed_hash['hash'], $cdef_id));
...
function xml_to_vdef($hash, &$xml_array, &$hash_cache) {
	...
	/* import into: vdef */
	$_vdef_id = db_fetch_cell_prepared('SELECT id FROM vdef WHERE hash = ?', array($hash));
	...
	if (is_array($xml_array['items'])) {
		while (list($item_hash, $item_array) = each($xml_array['items'])) {
			...
			unset($save);
			$_vdef_item_id = db_fetch_cell_prepared('SELECT id FROM vdef_items WHERE hash = ? AND vdef_id = ?', array($parsed_hash['hash'], $vdef_id));
...
function xml_to_data_input_method($hash, &$xml_array, &$hash_cache) {
	...
	$_data_input_id = db_fetch_cell_prepared('SELECT id FROM data_input WHERE hash = ?', array($hash));
	...
	if (is_array($xml_array['fields'])) {
		while (list($item_hash, $item_array) = each($xml_array['fields'])) {
			...
			unset($save);
			$_data_input_field_id = db_fetch_cell_prepared('SELECT id FROM data_input_fields WHERE hash= ? AND data_input_id = ?', array($parsed_hash['hash'], $data_input_id));
