	...
	if ($poller_id > 1) {
		if (remote_poller_up($poller_id)) {
			if (($rcnn_id = poller_push_to_remote_db_connect($poller_id, true)) !== false) {
				...
				poller_item_delete_for_host($int_device_ids, $rcnn_id, false);
...
function api_device_purge_deleted_devices() : void {
	...
	if (cacti_sizeof($devices)) {
		foreach ($devices as $d) {
			...
			poller_item_delete_for_host($d['id']);
				...
				if ($snmp_system === false || $snmp_system == 'U' || $snmp_error != '') {
					...
					if ($snmp_system == '' && $snmp_uptime === false) {
						...
						if ($snmp_uptime === false) {
							$snmp_uptime = 'U';
							print '<b>' . __('Uptime:') . '</b> ' . htmle($snmp_uptime) . '<br>';
						} else {
							...
							$snmp_uptime       = (string) $snmp_uptime;
