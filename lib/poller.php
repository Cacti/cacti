...
/**
 * Batch-prefetches the per-data-source RRD field-name metadata process_poller_output() and
 * rrdtool_function_update() need while walking a poller_output result set. The field-name
 * shape (which data_source_names exist, which are "unused"/orphaned from the active graph)
 * is a property of the data TEMPLATE, not of any one instance, so templated results are keyed
 * by data_template_id - every local_data_id sharing a template gets the same answer without
 * re-querying per instance. Only non-templated (data_template_id == 0) data sources, which
 * have no shared template at all, are queried and keyed per local_data_id. Used as part of
 * Cacti's lib functionality.
 *
 * @param array $local_data_ids Distinct local_data_ids in this batch.
 * @param array $data_template_id_by_id local_data_id => data_template_id map, used to pick the
 *   templated vs non-templated nt_rrd_field_names query and to key templated results by
 *   data_template_id instead of local_data_id.
 *
 * @return array Three maps: 'unused' and 'nt_by_template' keyed by data_template_id (templated
 *   data sources only), and 'nt_by_id' keyed by local_data_id (non-templated data sources only).
...
function process_poller_output(&$rrdtool_pipe, $remainder = 0) {
	...
	if (cacti_sizeof($results)) {
		...
		foreach ($results as $item) {
			...
			$rrd_update_array[$rrd_path]['data_template_id'] = $data_template_id;
