...
if (cacti_sizeof($parms) == 0) {
	...
	if (isset($groupId) && $groupId > 0) {
		...
		if ( db_fetch_cell("SELECT id FROM user_auth_group WHERE id=$groupId") ) {
    ...
    $groupName = db_fetch_cell("SELECT name FROM user_auth_group WHERE id=$groupId");
    /* This needs to be replace earlier up to define idName for each case 
     * Right now it only does hostname */
    $idName = db_fetch_cell("SELECT hostname FROM host WHERE id=$itemId");
