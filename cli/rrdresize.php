	...
	foreach($parms as $parameter) {
		...
		case "--data-template-id":
			...
			if (!is_numeric($data_template_id)) {
				echo "ERROR: You must supply a valid data template id to run this script!\n";
				exit(1);
			}else {
		...
		case "--backup":
			...
			if (!$backup_folder) {
				echo "ERROR: You must supply a valid backup folder to run this script!\n";
				exit(1);
			}
			if (!file_exists($backup_folder)) {
				if(mkdir($backup_folder, 0777, true) === false) {
					echo "ERROR: Cannot create backup folder: \"$backup_folder\"\n";
					exit(1);
				}
			}else {
				if(is_dir($backup_folder) === false) {
					echo "ERROR: Given backup folder: \"$backup_folder\" is not a directory\n";
					exit(1);
				}else {
					if(!is_writable($backup_folder)) {
						echo "ERROR: No write permission to backup folder: \"$backup_folder\" given.\n";
						exit(1);
					}
				}
			}
...
if($displayDataTemplates) {
	$data_templates = db_fetch_assoc("SELECT id, name FROM data_template ORDER BY id, name");
	echo "Known Data Templates: (id, name)\n";

	if (sizeof($data_templates)) {
		foreach($data_templates as $data_template) {
			echo $data_template["id"] . "\t" . $data_template["name"] . "\n";
		}
	}else {
		echo "ERROR: No data templates found.\n";
	}
	echo "\n";
	exit(0);
}
...
if(!$backup_folder) {
	echo "ERROR: You must define a backup folder!\n";
	exit(0);
}else {
...
if($scanned_directory['folders']) {
	...
	if(!mkdir($tmp_backup_folder)) {
		echo "ERROR: Unable to create a backup folder!\n";
		exit(0);		
	}
	...
	$db_hosts = db_fetch_assoc('SELECT DISTINCT host_id FROM data_local WHERE data_template_id = ' . $data_template_id);
	$dt_hosts = array();
	...
	$db_data_sources = db_fetch_assoc('SELECT DISTINCT local_data_id from data_template_data where data_template_id = ' . $data_template_id . ' AND data_source_path IS NOT NULL');
	...
	foreach($subfolders as $host_id => $host_data) {
		...
		if($host_data['files']) {
			...
			foreach($host_files as $host_file) {
				...
				$local_data = db_fetch_row("SELECT * FROM data_template_data WHERE data_source_path = '" . mysql_real_escape_string($search_pattern) ."'");
				...
				if($local_data) {
					...
					$data_template_ds = db_fetch_assoc("SELECT id, rrd_maximum, rrd_minimum, rrd_heartbeat, data_source_type_id, data_source_name FROM data_template_rrd WHERE hash != '' AND data_template_id = " . $local_data['data_template_id'] . " ORDER BY id");
					$data_template_ds_counter = sizeof($data_template_ds);
					$data_template_data_settings = db_fetch_assoc('select * from data_template_data_rra LEFT JOIN rra ON rra.id = data_template_data_rra.rra_id RIGHT JOIN rra_cf ON rra_cf.rra_id = rra.id where data_template_data_rra.data_template_data_id = ' . $local_data['id']);
...
function display_help() {
	echo "\nRRDfile Reassign Data Template Script 1.0\n";
	echo "A simple command line utility to analyse and reassign data template settings\nto RRDfiles based upon.\n\n";
	echo "usage: rrdresize.php --data-template-id=[ID] --backup=[PATH] [--dry-run] [--debug, -d]\n\n";
	echo "    --data-template-id          the numerical ID of the host\n";
	echo "    --backup                    path to backup original RRDfiles\n";
	echo "    --dry-run                   no boost cache update and no replacement of rrdfiles\n";
	echo "    --debug, -d                 show detail processing data\n";
	echo "    --list-data-templates       list all data templates\n\n";
}
