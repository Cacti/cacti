...
if (sizeof($parms)) {
	...
	if ($displaySites) {
		...
		if ($siteId && ($deviceMapRegex || $deviceMapWild || $ipMapRegex || $ipMapWild)) {
			if ($doMap && !$quiet) 	{ echoQuiet("Attempting to map devices to site ID: $siteId\n"); }
			elseif (!$quiet)	{ echoQuiet("Dry run - checking filters to map devices to site ID: $siteId\n"); }
...
function addSite() {
	...
	list($siteLatitude,$siteLongitude) = fixCoordinates($siteLatitude,$siteLongitude);			# Fix nasty DMS values
	...
	if ($siteData) { 
		...
		if (!$siteId) { 
			if (!$quiet) { echoQuiet("Error - couldn't find ID for site name: $siteName"); }
			exit;
		}
		
		$params = array();

		array_push($params,$siteName 	  ? $siteName 		: (isset($siteData[0]) ? $siteData[0]['name'] : ''));
		array_push($params,$siteAddr1 	  ? $siteAddr1 		: (isset($siteData[0]) ? $siteData[0]['address1'] : ''));
		array_push($params,$siteAddr2 	  ? $siteAddr2 		: (isset($siteData[0]) ? $siteData[0]['address2'] : ''));
		array_push($params,$siteCity 	  ? $siteCity 		: (isset($siteData[0]) ? $siteData[0]['city'] : ''));
		array_push($params,$siteState 	  ? $siteState 		: (isset($siteData[0]) ? $siteData[0]['state'] : ''));
		array_push($params,$siteZip 	  ? $siteZip 		: (isset($siteData[0]) ? $siteData[0]['postal_code'] : ''));
		array_push($params,$siteCountry   ? $siteCountry 	: (isset($siteData[0]) ? $siteData[0]['country'] : ''));
		array_push($params,$siteTimezone  ? $siteTimezone 	: (isset($siteData[0]) ? $siteData[0]['timezone'] : ''));
		array_push($params,$siteLatitude  ? $siteLatitude 	: (isset($siteData[0]) ? $siteData[0]['latitude'] : ''));
		array_push($params,$siteLongitude ? $siteLongitude 	: (isset($siteData[0]) ? $siteData[0]['longitude'] : ''));
		array_push($params,$siteAltname   ? $siteAltname 	: (isset($siteData[0]) ? $siteData[0]['alternate_id'] : ''));
		array_push($params,$siteNotes 	  ? $siteNotes 		: (isset($siteData[0]) ? $siteData[0]['notes'] : ''));
		array_push($params,isset($siteData[0]) ? $siteData[0]['id'] : 0);
		...
		$params=array();
		array_push($params,$siteName      ? $siteName           : "");
                array_push($params,$siteAddr1     ? $siteAddr1          : "");
                array_push($params,$siteAddr2     ? $siteAddr2          : "");
                array_push($params,$siteCity      ? $siteCity           : "");
                array_push($params,$siteState     ? $siteState          : "");
                array_push($params,$siteZip       ? $siteZip            : "");
                array_push($params,$siteCountry   ? $siteCountry        : "");
                array_push($params,$siteTimezone  ? $siteTimezone       : "");
                array_push($params,$siteLatitude  ? $siteLatitude       : "");
                array_push($params,$siteLongitude ? $siteLongitude      : "");
                array_push($params,$siteAltname   ? $siteAltname        : "");
                array_push($params,$siteNotes     ? $siteNotes          : "");
		
		db_execute_prepared("INSERT into sites 
					(name , address1 , address2 , city , state , postal_code , country , timezone , latitude , longitude , alternate_id , notes)
					VALUES (?,?,?,?,?,?,?,?,?,?,?,?)", $params);
...
function mapDevices($siteId, $doMap) {
	...
	if ($deviceMapRegex && !preg_match('/^\/.+\//',$deviceMapRegex)) 	{ $deviceMapRegex = '/^'.$deviceMapRegex.'$/';}	# Just in case the slashes aren't passed to us
	if ($ipMapRegex && !preg_match('/^\/.+\//',$ipMapRegex)) 	{ $ipMapRegex = '/^'.$ipMapRegex.'$/';}			# Make it more restrictive too - add the ^ and $ anchors if the regex isn't specified correctly to stop sillyness
	
	$deviceMapWild 	= $deviceMapWild ? '/'.str_replace('%','.+',$deviceMapWild) .'/' : "";		# Cheating and just expanding % into .+ regex matches to avoid having to do DB queries again
	...
	foreach ($devices as $device) {
		...
		if ($deviceMapRegex && (preg_match($deviceMapRegex,$deviceName))) {
			...
			else { array_push($matchedDevices,"$deviceName [$deviceId]"); }
		}
...
function geocodeAddress($siteAddr1,$siteAddr2, $siteCity, $siteZip, $siteCountry) {
	...
	if ($verbose || $debug) { echoQuiet("Geocode URL: $requestUrl\n"); }
	$result = fetchCurl($requestUrl);

	if ($result) { 
		$jsonResult = json_decode($result);
		
		if ($debug) { echoQuiet("Result was: ". print_r($jsonResult,1)); }
		if ($jsonResult && isset($jsonResult->results[0])) { 
			...
			if (!$quiet) { echoQuiet("Geocoded Coordinates: $latGeocode,$lngGeocode\n"); }
		}
		else {
			if (!$quiet) { echoQuiet("Error: Query to URL: $requestUrl failed.\n"); }
		}
	}
...
function displayHelp($errorMessage = null) {
	...
	$log = false;
	displayVersion();
	if ($errorMessage) { echoQuiet("$errorMessage\n\n"); }
...
function fetchCurl($url){
    ...
    if ($httpsProxy) {
	if ($verbose || $debug) { echoQuiet("Using HTTPS proxy: $httpsProxy\n"); }
