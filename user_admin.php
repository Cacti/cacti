...
function form_actions()
{
	...
	if (isset_request_var('associate_host')) {
		...
		if (get_nfilter_request_var('drp_action') == '2') { /* copy */
			...
			$template_user = db_fetch_row_prepared(
...
function form_save()
{
	...
	if ((isset_request_var('save_component_graph_perms')) && (!is_error_message())) {
		...
		$old_password = db_fetch_cell_prepared(
			'SELECT password
			FROM user_auth
			WHERE id = ?',
			...
			<script type='text/javascript'>
				...
				function checkPassword() {
					if ($('#password').val().length == 0) {
						...
						$.post('user_admin.php?action=checkpass', {
