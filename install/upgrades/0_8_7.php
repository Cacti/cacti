...
function upgrade_to_0_8_7() : void {
	...
	// speed up the poller
	db_install_add_key('host', 'index', 'disabled', ['disabled']);
	db_install_add_key('poller_item', 'index', 'rrd_next_step', ['rrd_next_step']);

	// speed up the UI
	db_install_add_key('poller_item', 'index', 'action', ['action']);
	db_install_add_key('user_auth', 'index', 'username', ['username']);
	db_install_add_key('user_auth', 'index', 'realm', ['realm']);
	db_install_add_key('user_log', 'index', 'username', ['username']);
	db_install_add_key('data_input', 'index', 'name', ['name']);

	// Add enable/disable to users
	db_install_add_column('user_auth', ['name' => 'enabled', 'type' => 'CHAR(2)', 'default' => 'on']);
	...
	if (db_table_exists('rra')) {
		...
		db_install_execute('INSERT INTO `rra_cf` VALUES (?,1), (?,3)', [$rrd_id, $rrd_id]);
	}
