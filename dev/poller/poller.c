...
void *poller(void *thread_args){
  ...
  while(1){
    ...
    if (current != NULL) {
      ...
      printf("snmp_get(%s,%s,%i,%s,%i)\n",entry->host, entry->community, entry->ver, entry->oid, worker->index);
      result = snmp_get(entry->host, entry->community, entry->ver, entry->oid, worker->index);
