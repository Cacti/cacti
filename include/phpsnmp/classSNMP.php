...
class SNMP {
	...
	function set_security($sec_level, $auth_protocol, $auth_passphrase, $priv_protocol, $priv_passphrase, $contextName, $contextEngineID) {
	...
	private function uniget($command, $oids) {
		...
		foreach ($oids as $oid) {
			$output[$oid] = $function_name($this->hostname, $this->community, $oid, $this->version, $this->username, $this->auth_pass, $this->auth_proto, $this->priv_pass, $this->priv_proto, $this->contextName, $this->port, $this->timeout, $this->retries, SNMP_POLLER);
		}
	...
	function walk($oid, $dummy = FALSE, $max_repetitions = 10, $non_repeaters = 0) {
		...
		$options_backup = $this->apply_options();
		$result = cacti_snmp_walk($this->hostname, $this->community, $oid, $this->version, $this->username, $this->auth_pass, $this->auth_proto, $this->priv_pass, $this->priv_proto, $this->contextName, $this->port, $this->timeout, $this->retries, $max_repetitions, SNMP_POLLER);
		if ($result === FALSE) {
			$this->errno = SNMP::TIMEOUT;
		}
		$this->apply_options($options_backup);
