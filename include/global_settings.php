...
$settings = array(
	...
	'general' => array(
		...
		'allow_unsafe_metachars' => array(
			...
			'description' => __('Historically, Cacti administrators created simple Data Input Methods that include commands like "ps -ef | grep something | wc -l" to represent a numeric values for use in Cacti Graphs.  In earlier versions of Cacti, this was permitted.  However, this is an unsafe practice as it is a gateway for third party users to publish insecure Data Templates.  In modern Cacti, these types of practices should be avoided.  However, we provide this option if you have older Data Input Methods that contain these unsafe Meta Characters and have not converted them to local scripts in the \'scripts\' directory.  Unsafe metacharacters for use in web applications include blank spaces, quotation marks, less-than signs (<), greater-than signs (>), percent signs (%), left curly brackets ({), right curly brackets (}), vertical bars (|), backslashes (\), circumflex or caret (^), and backticks (`).  Cacti continues to allow greater-than and less-than signs for things like <path_cacti> and for input parameters but if this option is unchecked, that is the limit of their permitted use.'),
			'default' => '',
			'method' => 'checkbox',
		),
