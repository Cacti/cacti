...
function form_save() {
	...
	if ((isset_request_var('save_component_data_source')) && (!is_error_message())) {
		...
		$save2['data_source_path']            = form_input_validate(get_nfilter_request_var('data_source_path'), 'data_source_path', '^[^\r\n]*$', true, 3);
	...
	if (((isset_request_var('id')) || (isset_request_var('new'))) && (empty($data['data_template_id']))) {
		...
		print "<div class='tableHeader'>
			...
			<div class='tableSubHeaderColumn right'>
				" . ((!isempty_request_var('id') && (empty($data_template['id']))) ? "<a class='linkOverDark cactiPostAction' href='" . html_escape('data_sources.php?action=rrd_add&id=' . get_request_var('id')) . "'>" . __('New') . '</a>&nbsp;' : '') . '
			</div>
		</div>';
