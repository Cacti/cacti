...
if queryType == "Downstream":
    ...
    for channel in downChannels:
        results.append(
            "StatsUnerrored%s:%s"
            % (channel, data["Stats"]["Total Unerrored Codewords"][channel])
        )
        results.append(
            "StatsCorrectable%s:%s"
            % (channel, data["Stats"]["Total Correctable Codewords"][channel])
        )
        results.append(
            "StatsUncorrectable%s:%s"
            % (channel, data["Stats"]["Total Uncorrectable Codewords"][channel])
        )
        del channel
