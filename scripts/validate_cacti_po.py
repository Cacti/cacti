...
def parse_po(path: Path) -> List[Entry]:
    # Use split('\n') instead of splitlines() because splitlines() splits on
    # Unicode newlines like U+0085 which can appear in UTF-8 translations.
    try:
        lines = path.read_text(encoding="utf-8").split("\n")
    except UnicodeDecodeError:
        # Fallback for files that might not be clean UTF-8
    ...
    def flush():
        ...
        if cur.msgid is not None or cur.msgstr or cur.comments:
            entries.append(cur)
        cur = Entry(); active_field = active_index = None
    for lineno, line in enumerate(lines, 1):
        line = line.rstrip("\r") # Handle CRLF if present
        if not line.strip(): flush(); continue
        if line.startswith("#,"):
            cur.comments.append(line)
            cur.flags.update(x.strip() for x in line[2:].split(",") if x.strip())
            continue
