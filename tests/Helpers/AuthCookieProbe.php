 ...
 *
 * check_auth_cookie() calls db_execute_prepared()/db_fetch_row_prepared()/
 * db_fetch_cell_prepared()/db_table_exists() with no explicit connection,
 ...
 * process ever sees. This matters because phpunit-cookie.xml runs the
 * integration scenarios together while other suites may load the real
 * database helpers first.
 *
...
foreach ($scenario['calls'] as $call) {
	...
	try {
		if ($call['type'] === 'check_auth_cookie' || $call['type'] === 'clear_auth_cookie') {
