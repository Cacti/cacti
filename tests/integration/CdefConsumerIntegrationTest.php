...
test('graph CDEF caching preserves valid empty and non-empty definitions', function ($cdef) : void {
	$state = rrdtool_normalize_graph_item_cdef($cdef);

	expect($state['cdef_invalid'])->toBeFalse()
		->and($state['cdef_cache'])->toBe($cdef);
})->with(array('empty definition' => '', 'resolved definition' => 'CURRENT_DATA_SOURCE,8,*'));
