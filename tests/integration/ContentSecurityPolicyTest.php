 ...
 * HTMX (default), Nonce, and None (unsafe-eval).
 *
...
function _csp_start_server($mode, $alternates = '', $reportUri = '') {
	...
	$cmd = array($php_bin, '-S', '127.0.0.1:' . (int) $port, '-t', $docroot, 'csp_fixture.php');
	...
	return array('proc' => $proc, 'port' => $port, 'pipes' => $pipes);
}
...
test('Nonce mode emits a nonce that matches the served script tag', function () {
	...
	try {
		...
		// the header nonce and the body <script nonce="..."> must be identical
...
test('Nonce mode is enforcing, not report-only', function () {
	$server = _csp_start_server('nonce');
	try {
		$resp = _csp_fetch($server['port']);
