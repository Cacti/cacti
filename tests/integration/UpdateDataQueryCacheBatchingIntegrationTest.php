 ...
 * tests/Unit/Database/DbColumnTypeCacheTest.php-style tests rely on implicitly
...
/* poller_update_poller_cache_from_buffer() records a last-changed timestamp on
   every flush. That used to be stubbed out behind function_exists(), but the
   stub only applied when nothing had already loaded lib/functions.php, and the
   unit suite always has by the time this file runs. The real function then ran
   instead, dereferenced CACTI_PATH_LIBRARY and fataled, and its extra query
   changed the counts below. Load the real one deliberately and give it what it
   needs, so this file behaves the same whatever ran before it. */
/* Guarded defines rather than require_once of include/global_constants.php:
   that file defines unconditionally, and sibling tests (Helpers/UnitStubs.php,
   CmdPhpPollerItemColumnCollisionIntegrationTest) already define a subset of
   the same names, so requiring it here emits "already defined" warnings on any
   ordering where one of those ran first. Guarded defines emit none either way. */
foreach ([
	'CACTI_WEB'          => false,
	'POLLER_ID'          => 1,
	'CACTI_PATH_BASE'    => dirname(__DIR__, 2),
	'CACTI_PATH_LIBRARY' => dirname(__DIR__, 2) . '/lib',
	'OPTIONS_CLI'        => 'config_options_array',
	'OPTIONS_WEB'        => 'sess_config_array',
] as $name => $value) {
	if (!defined($name)) {
		define($name, $value);
	} elseif (constant($name) !== $value) {
		throw new RuntimeException("$name has an incompatible value for the batching integration test");
	}
}

// An existing suite bootstrap may point logging at another valid directory.
...
/* after lib/functions.php: UnitStubs guards with function_exists(), so loading
   it first would make the real definitions a redeclaration fatal */
...
function batching_seed_poller_item_table() : FakeMySQLPDO {
	...
	// set_config_option() writes the last-changed timestamp here
