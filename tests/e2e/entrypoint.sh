...
# The 1.2.x CSP harness runs on PHP 7.4, while the committed Composer vendor
# tree may include a platform check generated by newer Composer metadata.
platform_check="${CACTI_ROOT}/include/vendor/composer/platform_check.php"
if [ -f "${platform_check}" ]; then
    log "disabling Composer platform check for PHP 7.4 e2e runtime"
    cat > "${platform_check}" <<'PHP'
<?php
// Disabled by the CSP e2e harness: this branch intentionally tests PHP 7.4.
PHP
fi
