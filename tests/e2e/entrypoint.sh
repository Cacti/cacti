...
printf '%s\n' \
	...
	"INSERT INTO login_providers (id, name, type, enabled, debug, is_default, allow_auth_cookies, user_id, parameters) VALUES (1, 'E2E LDAP', 1, 'on', '', 1, 'on', 0, '{\"server\":\"openldap\",\"port\":389,\"port_ssl\":636,\"proto_version\":3,\"network_timeout\":2,\"bind_timeout\":2,\"encryption\":0,\"tls_certificate\":2,\"referrals\":0,\"mode\":0,\"dn\":\"cn=<username>,ou=users,dc=example,dc=org\",\"group_dn\":\"\",\"group_attrib\":\"\",\"group_member_type\":0,\"search_base\":\"ou=users,dc=example,dc=org\",\"search_filter\":\"(cn=<username>)\",\"specific_dn\":\"\",\"specific_password\":\"\",\"claim_full_name\":\"\",\"claim_email\":\"\"}') ON DUPLICATE KEY UPDATE name=VALUES(name), enabled='on', is_default=1, user_id=0, parameters=VALUES(parameters);" \
