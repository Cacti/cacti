...
platform_check="${CACTI_ROOT}/include/vendor/composer/platform_check.php"
if [ -f "${platform_check}" ]; then
    log "disabling Composer platform check for the minimal e2e runtime"
    cat > "${platform_check}" <<'PHP'
<?php
// Disabled by the CSP e2e harness: this image doesn't install every
// extension composer.json declares (see entrypoint.sh).
PHP
fi
