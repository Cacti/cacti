...
test('xport returns a controlled empty result when rrdtool emits no metadata', function () use ($rrdSource) {
	...
	$emptyExportShape = <<<'REGEX'
/if \(!isset\(\$xport_array\['meta'\]\)\) \{.*'meta' => array\(.*'start'\s+=> \$xport_start,.*'columns'\s+=> 0,.*'legend'\s+=> array\(\),.*'title_cache'\s+=> \$graph\['title_cache'\],.*'data' => array\(\),/s
REGEX;

	expect($rrdSource)->toMatch($emptyExportShape);
});
