...
/**
 * 1.3 raises the minimum PHP version to 8.3. utility_php_verify_recommends()
 ...
 * constant, falling back to an 8.3-based default if it isn't defined.
 *
...
test('CACTI_PHP_VERSION_MINIMUM is 8.3.0', function () {
	expect(defined('CACTI_PHP_VERSION_MINIMUM'))->toBeTrue()
		->and(CACTI_PHP_VERSION_MINIMUM)->toBe('8.3.0');
});
...
test('utility.php and poller_maintenance.php fallbacks agree with CACTI_PHP_VERSION_MINIMUM', function () {
	...
	expect(CACTI_PHP_VERSION_MINIMUM)->toBe('8.3.0');
	expect(substr(CACTI_PHP_VERSION_MINIMUM, 0, 3))->toBe('8.3');
});
