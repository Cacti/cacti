...
function template() {
	...
	if (sizeof($template_list) > 0) {
		foreach ($template_list as $template) {
			...
			form_selectable_cell("<a class='linkEditMain' href='host_templates.php?action=template_edit&id=" . $template["id"] . "'>" . eregi_replace("(" . preg_quote($_REQUEST["filter"]) . ")", "<span style='background-color: #F8D93D;'>\\1</span>", $template["name"]) . "</a>", $template["id"]);
