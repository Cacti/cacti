...
if (cacti_sizeof($graph)) {
	...
	foreach ($graph as $g) {
		...
		$ds = db_fetch_assoc('SELECT *
			FROM data_local
			WHERE host_id=' . $g['host_id'] . '
		...
		$rrd_data = db_fetch_assoc('SELECT *
			FROM data_template_rrd
			WHERE local_data_id=' . $ds['id']);
		...
		*/
		...
		$graph_templates_items_wrong = db_fetch_assoc('SELECT id, task_item_id
			FROM graph_templates_item
			WHERE task_item_id!=' . $rrd_data[0]['id'] . '
			AND graph_template_id=' . $graph_template_id . '
			AND local_graph_id=' . $g['id'] . '
		...
		$query = 'UPDATE graph_templates_item
			SET task_item_id=' . $rrd_data[0]['id'] . '
			WHERE task_item_id!=' . $rrd_data[0]['id'] . '
			AND graph_template_id=' . $graph_template_id . '
			AND local_graph_id=' . $g['id'] . '
			AND id in (' . implode(',',$graph_templates_item) . ')';
