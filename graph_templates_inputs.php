	...
	if ((isset_request_var('save_component_input')) && (!is_error_message())) {
		...
		if (!is_error_message()) {
			...
			if ($graph_template_input_id) {
				...
				$mutation_failed = !db_execute_prepared('DELETE FROM graph_template_input_defs WHERE graph_template_input_id = ?', array($graph_template_input_id));

				if (!$mutation_failed && cacti_sizeof($selected_graph_items) > 0) {
					foreach ($selected_graph_items as $graph_template_item_id) {
						if (!db_execute_prepared('INSERT INTO graph_template_input_defs (graph_template_input_id, graph_template_item_id) VALUES (?, ?)', array($graph_template_input_id, $graph_template_item_id))) {
...
function input_remove() {
	...
	if (!graph_template_input_relationships_are_valid((int) get_request_var('id'), (int) get_request_var('graph_template_id'), array())) {
	...
	if (db_begin_transaction()) {
		$deleted_defs = db_execute_prepared('DELETE FROM graph_template_input_defs WHERE graph_template_input_id = ?', array(get_request_var('id')));
		$deleted_input = $deleted_defs && db_execute_prepared('DELETE FROM graph_template_input WHERE id = ? AND graph_template_id = ?', array(get_request_var('id'), get_request_var('graph_template_id')));
